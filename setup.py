...
import os, glob, sys
from InQantationlib import APPNAME,APPVERSION,AUTHOR,DESCRIPTION,YEAR,PAGE,EMAIL
			...
			if os.path.basename(f) == '__init__.py':
				...
				if (const_begin != -1) and (const_end != -1):
					...
					if self.install_data:
						replace_me = os.path.join(self.install_data,'share/InQantation')
					elif self.prefix:
						replace_me = os.path.join(self.prefix,'share/InQantation')
					if self.root[-1] == '/':
...
setup(
	...
	url = PAGE,
	packages = ['inqantationlib'],
	cmdclass={'install': install},
	data_files = data_files,
	scripts=['inqantation.py']
)
